    ...
    for (let t = 0; t < nTracks; t++) {
      ...
      while (p < end) {
        ...
        if (status === 0xff) {
          ...
          if (type === 0x90 && vel > 0) { (active[key] || (active[key] = [])).push({ ticks: tick, velocity: vel }); }
          else { const q = active[key]; if (q && q.length) { const a = q.shift(); T.raw.push({ ticks: a.ticks, durTicks: tick - a.ticks, midi: pitch, velocity: a.velocity }); } }
